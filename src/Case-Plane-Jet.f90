...
module planejet
  ...
  PUBLIC :: init_planejet, boundary_conditions_planejet, postprocess_planejet, &
            visu_planejet, visu_planejet_init

contains

  subroutine boundary_conditions_planejet (ux,uy,uz,phi)
    ...
    return
  end subroutine boundary_conditions_planejet
  ...
  subroutine inflow (phi)
    ...
    do k=1,xsize(3)
       do j=1,xsize(2)
          ...
          bxx1(j,k)=half*(u1+u2)+half*(u1-u2)*tanh((one-two*sqrt(y**two))/four/theta_jet) &
       ...
       do is=1, numscalar
          do k=1,xsize(3)
             do j=1,xsize(2)
                if (istret.eq.0) y=(j+xstart(2)-2)*dy-yly/2.
                if (istret.ne.0) y=yp(j+xstart(2)-1)-yly/2.
                phi(1,j,k,is)=cp(is)*(half+half*tanh((one-two*sqrt(y**two))/four/theta_jet))
             enddo
          enddo
       enddo
    endif
  ...
  subroutine init_planejet (ux1,uy1,uz1,phi1)
    ...
    if (iscalar==1) then
       do k=1,xsize(3)
          do j=1,xsize(2)
             ...
             do i=1,xsize(1)
                do is=1,numscalar
                   phi1(i,j,k,is) = cp(is)*(half+half*tanh((one-two*sqrt(y**two))/four/theta_jet))
                enddo
             enddo
          enddo
       enddo
    ...
    return
  end subroutine init_planejet

  subroutine postprocess_planejet(ux1,uy1,uz1,pp3,phi1,ep1)
  ...
  end subroutine postprocess_planejet

  subroutine visu_planejet_init (visu_initialised)
  ...
  end subroutine visu_planejet_init
  
  subroutine visu_planejet(ux1, uy1, uz1, pp3, phi1, ep1, num)
